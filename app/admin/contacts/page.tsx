...
import { Mail, MessageSquare, Tag, Calendar, Eye, Trash2, Reply, Loader2 } from "lucide-react";
      ...
      {loading ? (
        ...
        <div className="grid lg:grid-cols-5 gap-6">
          <div className="lg:col-span-2 space-y-3">
            {contacts.length === 0 && (
              <p className="text-center text-[#94A3B8] text-sm py-12">No leads yet.</p>
            )}
            {contacts.map((c) => (
              <button
                key={c.id}
              ...
              >
                <div className="flex items-start justify-between gap-2 mb-2">
                  ...
                  <span className={`px-2 py-0.5 text-xs font-medium rounded-full flex-shrink-0 ${statusConfig[c.status].color}`}>
                ...
                <div className="flex items-center gap-3 mt-2 text-xs text-[#94A3B8]">
                  <span className="flex items-center gap-1"><Tag size={10} />{c.budget}</span>
                  <span className="flex items-center gap-1"><Calendar size={10} />{formatDate(c.created_at)}</span>
                </div>
              </button>
            ))}
          </div>
            ...
            {selected ? (
              <div className="bg-white rounded-2xl border border-[#E2E8F0] p-6">
                <div className="flex items-start justify-between gap-4 mb-6 pb-5 border-b border-[#F1F5F9]">
                  <div>
                    <h2 className="font-semibold text-[#0F172A] text-lg">{selected.name}</h2>
                ...
                <div className="grid grid-cols-2 gap-4 mb-6">
                  <div className="flex items-center gap-3 p-3 bg-[#F8FAFC] rounded-xl">
                    ...
                    <div>
                      ...
                      <p className="text-sm font-medium text-[#0F172A]">{selected.budget}</p>
                    </div>
                  </div>
                  <div className="flex items-center gap-3 p-3 bg-[#F8FAFC] rounded-xl">
                    ...
                    <div>
                      ...
                      <p className="text-sm font-medium text-[#0F172A]">{formatDate(selected.created_at)}</p>
                    </div>
                  </div>
                </div>

                <div className="mb-6">
                  <p className="text-xs font-medium text-[#94A3B8] uppercase tracking-wide mb-2">Message</p>
                ...
                <div>
                  <p className="text-xs font-medium text-[#94A3B8] uppercase tracking-wide mb-2">Quick Reply</p>
              ...
              <div className="h-64 bg-white rounded-2xl border border-[#E2E8F0] flex items-center justify-center">
                <div className="text-center">
                  ...
                  <p className="text-sm text-[#94A3B8]">Select a lead to view details</p>
                </div>
              </div>
            )}
          </div>
